...
注释只配对未被奇数反斜线转义的 `%%`；未闭合 delimiter 保持字面源码，`%%%%` 为空注释，连续 `%` 的余留字符保持可见。独立注释可跨空行作为一个 Live Preview 编辑块，行内代码、围栏代码和真实块边界后的缩进代码始终优先于注释解析，代码范围内同形的 `%%` 也不会提前关闭外层注释；段落内的四空格续行仍按普通正文处理。引用容器会逐层移除 `>` 后递归识别其中的围栏与缩进代码，并把字面范围映射回原始 Markdown 偏移。
