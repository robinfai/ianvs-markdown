...
完整纯文本 `aliases` 列表使用不带标签底色的可编辑 chip：删除立即写回，删除最后一项后保留精确空属性 `aliases:`；从空值输入后按 Tab 会生成 block list，并同步规范化其他安全的 top-level inline scalar list。typed、嵌套、截断或超限 aliases 仍保持只读。
