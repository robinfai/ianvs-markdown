...
- 对齐 Obsidian Border 主题点阵、虚线框、亮暗语法色、可复制语言标签、四列 Tab（含 1–3 空格后跨列 Tab）、折行和活动行轨道的代码块
...
- 可选 YAML front matter 信息卡和可折叠文档大纲
...
- 源码保真的原地编辑、Markdown 快捷键，以及按 Obsidian 层级退出、引用续行、marker 后 Tab 边界和空行分隔子块工作的列表自动续写；松散列表仍按单项编辑并保持嵌套引导线深度，bullet 或有序 delimiter 改变时重新开始源码列表组
...
注释只配对未被奇数反斜线转义的 `%%`；未闭合 delimiter 保持字面源码，`%%%%` 为空注释，连续 `%` 的余留字符保持可见。独立注释可跨空行作为一个 Live Preview 编辑块，行内代码、围栏代码和真实块边界后的缩进代码始终优先于注释解析，代码范围内同形的 `%%` 也不会提前关闭外层注释；段落内的四空格续行仍按普通正文处理。引用容器会逐层移除 `>` 后递归识别其中的围栏与缩进代码；有序、无序、空项、嵌套和 Tab 分隔列表也会按 CommonMark 内容缩进逐项投影，因此列表内代码的脚注、注释与块 ID 同形文本保持字面值，并把范围精确映射回不含容器前缀的原始 Markdown 偏移。
